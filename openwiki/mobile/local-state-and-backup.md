...
| AsyncStorage `gymnasia.mobile.chat.system_prompt.v3` | Prompt remoto validado, hash, candidato, deployment y ámbito | No | Solo Staging/Production; alternativa tras fallar el asset de release validado. La caché v2 heredada ya no se acepta. |
| AsyncStorage `gymnasia.mobile.health_safety.runtime.v1` | Overlay sanitario, hash, candidato, deployment y ámbito | No | Solo Staging/Production; se verifica el hash y se combina de forma monotónica con la política compilada. |
| AsyncStorage `gymnasia.mobile.health_safety.consent.v1` | Consentimiento versionado para el evaluador sanitario por proveedor | No | No se exporta ni importa; el fallo del evaluador conserva la decisión local. Consulte [Seguridad sanitaria y transparencia de IA](../agent/health-safety.md). |
