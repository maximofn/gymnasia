...
La ruta manual de alimento desconocido ya no llama a un escritor de GitHub desde el cliente. Crear o editar una comida actualiza el estado de React y su efecto normal de persistencia de `LocalStore`; no espera una incidencia ni almacena un ID de procedencia en `DietItem`. La única salida de feedback actual es `create_feature_issue` a través del [Worker de feedback](../services/feedback-worker.md); no reutilices esa frontera para enviar datos de comidas sin ampliar explícitamente su contrato de privacidad.
...
Si el nombre extraído coincide con una entrada del repositorio y los gramos son positivos, los datos nutricionales del repositorio sustituyen la estimación. De lo contrario, se almacenan los totales extraídos. Las recetas y productos comerciales desconocidos no crean por sí solos feedback ni solicitudes a GitHub. La persistencia del guardado del estimador sigue siendo independiente de cualquier feedback: el elemento ordinario de dieta se confirma localmente.
