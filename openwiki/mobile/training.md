...
Una instantánea profunda anterior a la sesión permite realizar una acción posterior a la finalización para restaurar los cambios de la plantilla conservando sus identidades originales. `buildTemplateSeriesSignature` compara el orden y los IDs de ejercicios y series, el tipo, las repeticiones, el peso, el descanso, los tres campos de tempo y todos los campos de cada mini-serie, incluidos sus enlaces de catálogo; las cadenas se comparan después de recortar espacios exteriores. Cuando cambia esa firma, el modal de finalización puede conservar las modificaciones o llamar a `revertWorkoutTemplateChangesAfterSession`. Cerrar el modal conserva las modificaciones canónicas.
...
1. **La restauración compara series, no toda la plantilla.** La firma incluye todo el contenido funcional de las series y mini-series, pero deliberadamente excluye el nombre, la categoría, el icono, la duración y los metadatos del ejercicio. Un cambio limitado a esos campos no activa por sí solo la opción de restaurar al finalizar.
...
4. **Las mini-series no tienen edición visual completa durante una sesión.** El selector de tipo conserva las mini-series existentes y puede crear la primera, y la comparación de dominio cubre todos sus campos, pero la sesión activa no ofrece controles específicos para editar visualmente cada mini-serie.
...
`train-series-migration.e2e.mjs` siembra series con tempo, drop-set y un tipo desconocido y verifica que sobreviven a la hidratación y recarga. `train-series-operations.e2e.mjs` cubre la duplicación de series, ejercicios y rutinas avanzadas, los IDs regenerados, el cambio de tipo durante una sesión y la opción final de conservar o restaurar la plantilla. Se ejecutan mediante `npm run test:train:series:e2e` y `npm run test:train:series-operations:e2e`.

Vitest cubre además cada operación pura de `workoutTemplateOperations.ts`, un contrato que impide duplicar la lógica en `App.tsx` y propiedades con 500 casos por prueba para independencia de objetos, renovación de IDs, reasignación de referencias y sensibilidad de la firma. Siguen fuera de estos recorridos la temporización nativa en segundo plano, las notificaciones y el audio, algunos flujos de pausa y finalización parcial, los fallos del repositorio, los períodos estadísticos, la interacción de copia de seguridad e importación y las rutinas creadas por el agente.
