...
| Normalización de plantillas | `buildSeriesFromLegacyExercise`, `normalizeStoredExerciseSeries`, `seriesToLegacySets`, `normalizeStore`, `resolveTrainingCategory`, `normalizeTemplateIcon` | Migra los `sets` heredados, valida y conserva series avanzadas, proporciona valores predeterminados y normaliza la categoría, el icono, la duración y los metadatos de los ejercicios |
| Copia y edición de plantillas | `cloneWorkoutTemplateSnapshot`, `duplicateWorkoutTemplate`, `duplicateWorkoutExercise`, `duplicateExerciseSeries`, `createSeriesAfter`, `changeExerciseSeriesType` y los controladores de `App.tsx` | Centraliza copias profundas, regeneración de identidades y cambios de tipo antes de modificar el array canónico `LocalStore.templates` |
...
5. Las plantillas pueden clonarse, moverse hacia arriba o eliminarse de la lista de rutinas. `cloneWorkoutTemplateSnapshot` crea una copia profunda que conserva todos los IDs para poder restaurarla. Las operaciones de duplicación regeneran los IDs de plantilla, ejercicios, series y mini-series, clonan los enlaces de catálogo sin compartir referencias y, al duplicar una rutina completa, reasignan los `exercise_id` internos de las superseries hacia los ejercicios nuevos.
...
El movimiento del puntero y el enfoque de ejercicios están deshabilitados durante el descanso. Durante la sesión, los usuarios pueden modificar los campos y el tipo de las series, añadir, eliminar o reordenar series, añadir, reordenar o eliminar ejercicios y crear un ejercicio personalizado. `createSeriesAfter` conserva tipo, tempo, valores y mini-series de la serie anterior con IDs nuevos. `changeExerciseSeriesType` mantiene ocultas las mini-series al pasar a un tipo simple, las recupera al volver a un tipo compuesto y crea una inicial con los valores visibles si nunca existieron. Estas acciones editan la misma plantilla canónica y ajustan o vuelven a normalizar las claves y los recuentos de la sesión. Los controladores de eliminación correspondientes protegen la existencia de al menos un ejercicio y de al menos una serie por ejercicio, aunque otras rutas de modificación y la hidratación siguen requiriendo normalización.
...
| `gymnasia.mobile.training.session_template_draft.v1` | Borrador versionado de la rutina durante la sesión | Se escribe al modificar el borrador y se elimina después de persistir la resolución |
...
La copia de seguridad manual **no** incluye la sesión activa, su instantánea base ni su borrador. Al confirmar una importación, `applyPendingImport` llama explícitamente a `setActiveWorkoutSession(null)` después de sustituir las particiones respaldadas. A continuación, los efectos de persistencia eliminan las tres claves de trabajo en curso; por tanto, la importación finaliza la sesión en lugar de reconciliarla con las plantillas importadas. Consulta [Estado local y copia de seguridad](./local-state-and-backup.md) antes de modificar estos límites.
...
Se puede llegar a `finishWorkoutSession` desde el puntero final o desde la acción explícita de finalización, incluso desde los estados de pausa o descanso y antes de que todas las series estén completadas. El resumen tiene una identidad determinista derivada de la sesión y usa la fecha de la resolución pendiente, por lo que reintentar después de un resultado ambiguo no duplica el historial.

La sesión trabaja sobre una copia profunda separada y nunca modifica por sí sola la plantilla canónica. `buildWorkoutTemplateRevision` y `diffWorkoutTemplates` comparan todos los campos funcionales de rutina, ejercicios, series y mini-series, ignorando únicamente sellos técnicos y espejos heredados derivados. Al finalizar o abandonar con cambios, la sesión queda pausada con una resolución persistida y el modal exige conservar el borrador, mantener la rutina actual o seguir entrenando. Si la rutina canónica cambió en paralelo, el modal lo indica y no sobrescribe sin una segunda elección explícita. La plantilla y el historial se confirman de forma duradera antes de eliminar las tres claves de trabajo en curso.
...
- El editor modifica una copia profunda. Solo “Guardar cambios” confirma una transacción duradera; cancelar o volver atrás conserva la plantilla original.
...
`train-series-migration.e2e.mjs` siembra series con tempo, drop-set y un tipo desconocido y verifica que sobreviven a la hidratación y recarga. `train-series-operations.e2e.mjs` cubre cancelación y guardado del editor, creación sin rutinas fantasma, duplicación profunda, aislamiento de cambios de sesión, conservar o mantener la rutina, abandono, y recuperación del borrador y de una resolución pendiente después de recargar. Se ejecutan mediante `npm run test:train:series:e2e` y `npm run test:train:series-operations:e2e`.

Vitest cubre además las operaciones puras de plantillas, la revisión funcional completa, el diff, el reducer transaccional, validación, conflicto, serialización del borrador y propiedades sobre cancelación/aplicación. Siguen fuera de estos recorridos la temporización nativa en segundo plano, las notificaciones y el audio, algunos flujos de pausa y finalización parcial, los fallos del repositorio, los períodos estadísticos, la interacción de copia de seguridad e importación y las rutinas creadas por el agente.
...
4. Mantén las copias, duplicaciones, creación posterior, cambio de tipo y firma dentro de `workoutTemplateOperations.ts`; el contrato de fuente impide reintroducirlas en `App.tsx`.
5. Comprueba que las instantáneas preservan IDs, que las duplicaciones los regeneran sin compartir objetos, que las referencias internas de superseries se reasignan y que cualquier campo funcional cambia la firma.
6. Valida una rutina creada desde la interfaz de usuario y otra creada por el agente y, después, ejecuta las pruebas deterministas y las pruebas E2E de entrenamiento y series avanzadas.
