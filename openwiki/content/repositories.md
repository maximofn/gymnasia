...
`prompts/AGENTS.md` también es una entrada en tiempo de ejecución alojada en el repositorio, aunque no forma parte de los catálogos JSON de alimentos y ejercicios. `apps/mobile/App.tsx::loadChatSystemPrompt` lo obtiene de GitHub Raw con una consulta de marca de tiempo durante la inicialización del chat y nuevamente con cada envío; `sendMessage` pasa el resultado como instrucción del sistema del agente. Una respuesta correcta y no vacía se almacena en caché en AsyncStorage bajo `gymnasia.mobile.chat.system_prompt.v1`. Un fallo de obtención, HTTP o lectura de texto recurre primero a ese valor almacenado en caché; si la caché no existe, no puede leerse o está vacía, se recurre al `DEFAULT_CHAT_SYSTEM_PROMPT` integrado. Una respuesta remota correcta pero vacía se normaliza a ese valor predeterminado integrado antes de almacenarse en caché. Esto significa que las modificaciones pueden cambiar el comportamiento del agente sin publicar una nueva versión de la aplicación, mientras que los clientes sin conexión pueden seguir usando la última instrucción almacenada en caché. Consulte [Tiempo de ejecución del agente](../agent/runtime.md) para conocer la ubicación de las instrucciones en el ciclo de vida de envío y del bucle de herramientas.
