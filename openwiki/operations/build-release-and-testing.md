...
| Política sanitaria, esquemas, snapshots y casos deterministas | `npm run check:health-safety` | `policy/health-safety/`, el bloque gestionado del prompt y los snapshots móviles permanecen sincronizados sin red ni una evaluación LLM autorizadora. |
| Pruebas de la política sanitaria | `npm run test:health-safety` | Casos, validadores y generación de la política sanitaria. |
| Política de privacidad y artefactos legales | `npm run check:legal` | Las fuentes `docs/legal/` generan los artefactos publicados y el módulo de copia sin deriva. |
| Contratos y validadores legales | `npm run test:legal` | Paridad entre interfaz, páginas públicas, versión/digest y límites de datos iniciales. Consulte [Política de privacidad y artefactos legales](privacy-and-legal-policy.md). |
