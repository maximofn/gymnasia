...
| Esquema canónico | `AgentToolDefinition`, `ToolInputSchema`, `AGENT_TOOL_DEFINITIONS`, `AGENT_TOOL_NAMES`, `CHAT_TOOLS` | Definir un catálogo único de 13 herramientas, sus efectos y los tres formatos de intercambio. |
...
4. La política base efectiva depende del canal. Desarrollo (`Local`) usa solo el snapshot integrado. Staging y producción resuelven un deployment exitoso `gymnasia-policy`, aceptan únicamente el asset `policy.md` de su release permitida y verifican su SHA-256; ante fallo usan una caché AsyncStorage válida del mismo entorno/canal y, por último, el snapshot integrado. `selectChatSystemPrompt` valida contenido y tipo antes de cachearlo. La clave exacta y en minúsculas de memoria personal `debug`, cuando tiene un valor no vacío, se anexa después bajo `## Instrucciones de depuracion`; no constituye un rol independiente del proveedor. Consulte [Seguridad sanitaria y transparencia de IA](health-safety.md) para el contrato de deployment, caché y composición local de política.
...
`prompts/AGENTS.md` es la fuente de la política de chat, mientras que `apps/mobile/agent/generated/chatSystemPrompt.generated.ts` es su snapshot integrado derivado. Fuera de `Local`, `loadChatSystemPrompt` obtiene la política desde el asset de release anunciado por un deployment validado, no desde GitHub Raw arbitrario; la caché es un recurso de reserva con identidad de entorno/canal, no una sustitución local de mayor prioridad. `composeAiSystemPrompt` añade después la política local de identidad de IA, por lo que una política remota no puede presentarlo como humano o profesional. Consulte [Seguridad sanitaria y transparencia de IA](health-safety.md).

La mutabilidad está limitada por el artefacto versionado: una nueva política remota solo se acepta desde una release cuyo candidato, URL, hash, canal y estado de deployment satisfacen `fetchActivePolicyDeployment`; el cliente conserva una caché con hash e identidad de entorno/canal y nunca usa la caché heredada v2. El snapshot integrado sigue siendo la última alternativa y debe mantenerse sincronizado con `prompts/AGENTS.md` mediante `npm run sync:chat-prompt`; `npm run check:chat-prompt` y las pruebas de `chatSystemPrompt.test.ts` cubren validación de contenido, hash, caché y alternativas. La composición local de identidad descrita en [Seguridad sanitaria y transparencia de IA](health-safety.md) se aplica a cada prompt final.

El campo exacto de memoria `debug` continúa siendo una política mutable local: se anexa después de la política base y puede afectar todas las conversaciones. Ninguna capa de deployment transforma ese contenido en dato confiable. Mantenga su revisión separada y no confunda la verificación del asset remoto con validación de memoria personal o de argumentos de herramientas.
...
| `create_feature_issue` | `title`, `summary` | Envía un borrador saneado al cliente de feedback inyectado. No admite conversación literal y solo informa creación cuando el Worker devuelve número y URL verificables; consulte [Worker de feedback](../services/feedback-worker.md). |
...
- Las salidas de las herramientas son cadenas incluso cuando semánticamente contienen JSON. No existe transacción, reversión ni deduplicación de identificadores de llamadas; `create_feature_issue` delega una clave de idempotencia derivada al Worker, pero eso no hace idempotentes las demás herramientas.
...
`create_feature_issue` usa el cliente inyectado `createFeedbackIssueClient`, no un escritor de GitHub dentro de `App.tsx`. El cliente transmite únicamente las cinco claves cerradas del contrato, aplica un plazo de 15 s y mapea la respuesta a un resultado discriminado. Un estado `created` exige HTTP 2xx junto con número positivo y URL de issue verificable; el endpoint ausente en desarrollo degrada la función sin afectar los datos locales. El Worker conserva repositorio, etiquetas, deduplicación y credenciales. Consulte [Worker de feedback e incidencias verificables](../services/feedback-worker.md).

`sendMessage` reintenta una llamada completa a `callProviderChatAPIWithTools` hasta tres veces para los mensajes que coincidan con `failed to fetch`, `network`, `timeout`, `econnrefused`, `econnreset`, `overloaded`, `529`, `503` o `429`, esperando 2 s y después 4 s. Las herramientas de escritura local pueden repetirse porque ni el reintento externo ni los bucles registran los identificadores de llamadas completadas, por lo que presentan una semántica de **al menos una vez** durante los reintentos. `create_feature_issue` reduce su riesgo de duplicación mediante la clave estable que el Worker reserva antes del upstream, pero no convierte el resto de la solicitud ni las escrituras locales en operaciones exactamente una vez.
...
| Llamada actual a `create_feature_issue` | El cliente informa creación solo con número y URL verificables; errores de transporte, plazo, servidor, límite o endpoint no configurado se devuelven al modelo como resultado no creado. |
| Una dependencia del controlador genera una excepción | Se propaga por el bucle y la política de reintentos; los efectos secundarios anteriores se conservan. El cliente de feedback convierte sus propios fallos de transporte y plazo en resultados discriminados. |
