...
- **Agente:** [Entorno de ejecución del agente](agent/runtime.md), [Seguridad sanitaria y transparencia de IA](agent/health-safety.md), [Configuración de proveedores](agent/provider-configuration.md) y [Streaming de proveedores y continuación de herramientas](agent/provider-streaming.md).
...
| Cambiar clasificación sanitaria, consentimiento del evaluador, streaming seguro, herramientas permitidas, política desplegada o divulgación de IA | [Seguridad sanitaria y transparencia de IA](agent/health-safety.md) | `classifyHealthSafetyText`, `healthSafetyToolAllowed`, `createHealthSafeStreamGate`, `loadHealthSafetyPolicy`, `composeAiSystemPrompt` | `npm run check:health-safety && npm run test:health-safety`; añadir `npm run check:chat-prompt` si cambia el prompt |
...
| Cambiar la política de privacidad, el descargo o las páginas legales publicadas | [Política de privacidad y artefactos legales](operations/privacy-and-legal-policy.md) | `docs/legal/privacy-policy.{es,en}.md`, `buildArtifacts`, `LegalFooter` | `npm run check:legal && npm run test:legal` |
...
| Contrato del agente | `apps/mobile/agent/toolDefinitions.ts::AGENT_TOOL_DEFINITIONS` y `CHAT_TOOLS` | Definiciones canónicas de las 13 herramientas, sus efectos y los esquemas de comunicación de los proveedores |
...
- **Los secretos dependen de la plataforma.** Las claves de los proveedores utilizan SecureStore cuando está disponible, pero el mecanismo alternativo para navegadores/plataformas no compatibles puede dejarlas en el almacenamiento local ordinario. El desarrollo web también puede replicar un almacén sin depurar en `apps/mobile/.dev-store.json`; nunca lo confirmes en el repositorio.
- **El prompt del entrenador es una política mutable y versionada.** Desarrollo usa el snapshot integrado; staging/producción aceptan solo un asset de release anunciado por un deployment exitoso de GitHub, con hash SHA-256, caché aislada por canal y alternativa integrada. La clave exacta de memoria personal `debug` se añade como texto privilegiado al prompt. Consulta [Seguridad sanitaria y transparencia de IA](agent/health-safety.md) antes de cambiar esa cadena.
...
- **La creación de feedback no escribe desde el cliente.** `create_feature_issue` envía solo título, resumen y una clave de idempotencia al Worker configurado; la aplicación solo informa creación si recibe número y URL verificables. En desarrollo, el endpoint no está configurado por defecto. Consulte [Worker de feedback e incidencias verificables](services/feedback-worker.md).
