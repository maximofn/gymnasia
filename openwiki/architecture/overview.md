...
flowchart TD
    ...
    Agent --> Prompt["Política de chat de release versionada"]
...
| Política mutable del agente | `prompts/AGENTS.md`, `agent/chatSystemPromptRuntime.ts::loadChatSystemPrompt`, `agent/policyDeployment.ts::fetchActivePolicyDeployment` | En Staging/Production seleccionar un asset de release verificado por deployment y hash, después caché aislada y finalmente snapshot integrado; en Local usar solo el snapshot; añadir el campo exacto `debug` de memoria personal | GitHub Deployments/releases, caché de AsyncStorage y memoria personal |
...
La aplicación obtiene dos clases de datos de GitHub arquitectónicamente distintas. Los repositorios `ejercicios`, `alimentos`, `productos_comerciales` y `recetas` son catálogos de referencia servidos desde GitHub Raw. `prompts/AGENTS.md` es una política privilegiada mutable, pero Staging/Production solo la aceptan como asset `policy.md` de una release anunciada por un deployment exitoso y comprobada por hash; su alternativa es una caché del mismo entorno/canal y después el snapshot integrado. Local no consulta la red para esta política. Un campo de memoria personal no vacío con la clave exacta y sensible a mayúsculas y minúsculas `debug` se añade al prompt del sistema seleccionado. Consulte [Ejecución del agente](../agent/runtime.md) y [Seguridad sanitaria y transparencia de IA](../agent/health-safety.md) para el contrato y los límites de esa cadena.
...
3. **Los catálogos remotos son datos de referencia, no almacenamiento del usuario ni políticas.** Los archivos JSON agregados y las imágenes alojados en GitHub Raw enriquecen los registros locales; sus fallos no deben interpretarse como la pérdida de datos de usuario persistidos localmente. El prompt sigue una cadena separada y versionada: release validada → caché aislada → snapshot integrado fuera de Local, y solo snapshot en Local. Consulte [Seguridad sanitaria y transparencia de IA](../agent/health-safety.md).
4. **El feedback cruza una frontera específica.** `create_feature_issue` no usa una credencial de GitHub en el cliente: envía un borrador limitado al Worker configurado y solo trata la respuesta como creada si contiene una referencia verificable. No modele GitHub Issues como un backend del estado del producto; consulte [Worker de feedback e incidencias verificables](../services/feedback-worker.md).
...
- `npm run test:agent:e2e` exporta la aplicación web, inicializa el almacenamiento del navegador, intercepta los catálogos de GitHub Raw, el deployment/asset de política y OpenAI, y valida el flujo de interfaz a SSE, herramienta y segunda ronda en `apps/mobile/scripts/agent-chat.e2e.mjs`.
...
- **Disponibilidad remota, divergencia de esquemas y política mutable:** los catálogos sin procesar de GitHub y las API de terceros pueden degradar funcionalidades debido a cambios de disponibilidad o respuesta. El prompt privilegiado se limita a releases validadas, hashes, canal y caché aislada, pero un deployment remoto sigue pudiendo cambiar el comportamiento sin una publicación móvil. La memoria `debug` continúa siendo texto local privilegiado y no se vuelve confiable por esa validación. Consulte [Seguridad sanitaria y transparencia de IA](../agent/health-safety.md).
