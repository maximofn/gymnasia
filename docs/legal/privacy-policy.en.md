...
and encrypted credentials left by removed features. It does **not** remove:
the assistant's memory, the foods you created, your preferences, the debug log, any
backups you exported, or the downloaded catalogues. Work is under way to make this
action delete everything it promises; until then, this policy describes its actual
behaviour.
